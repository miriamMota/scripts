...
## var2comp = vector amb el nom de les variables que es volen comparar amb "var.cont". (poden ser continues o factor)
## dat = data.frame que conté les variables var.cont, var2comp 
...
comp.continues <- function(var.cont, var2comp, dat, nround = 3, correlation = FALSE, corr.meth = "Spearman"){
  res <- list()
  rm(df.res) ;test.us <- NULL
  for (j in 1:length(var2comp)) {
    p.val <- NULL; cor.r <- NA; # ;med1 <- NA; med2 <- NA
    for (i in 1:length(var.cont)) {
      if ((class(dat[ ,var2comp[j]]) == "integer") | (class(dat[,var2comp[j]]) == "numeric")) {
        cr.ts <- cor.test(dat[,var.cont[i]], dat[ ,var2comp[j]], method = tolower(corr.meth) )
        if (correlation) {p.val[i] <- paste(cr.ts$estimate, "(", cr.ts$p.value, ")")
        }else {p.val[i] <- cr.ts$p.value}
        test.us[j] <- corr.meth
      }
      
      if ((class(dat[ ,var2comp[j]]) == "factor")) {
        if (length(levels(dat[ ,var2comp[j]])) == 2 ) {
          wx.ts <- wilcox.test(dat[,var.cont[i]] ~ dat[,var2comp[j]] )
          p.val[i] <- wx.ts$p.value
          # aggregate(dat[,var.cont[i]] ~ dat[ ,var2comp[j]] , data = dat, FUN= "median" )
          test.us[j] <- "U Mann-Withney"
        }
        if (length(levels(dat[ ,var2comp[j]])) > 2 ) {
          kr.ts <- kruskal.test(dat[,var.cont[i]] ~ dat[,var2comp[j]] )
    ...
    if (!exists("df.res")) {
      ...
      colnames(df.res)[j] <- ifelse(test.us[j] == corr.meth, 
                                    paste(gsub("_"," ",var2comp[j]), "$ _1$"), 
                                    ifelse(test.us[j] == "U Mann-Withney", 
                                           paste(gsub("_"," ",var2comp[j]), "$ _2$"), 
                                           paste(gsub("_"," ",var2comp[j]), "$ _3$")))
  ...
  df.res.adj <- apply(df.res, 2, function(x)p.adjust(x, method = "BH") )
